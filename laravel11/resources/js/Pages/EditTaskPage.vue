<template>
  ...
  export default {
    props: {
      task: Object, // Reçoit les données de la tâche à modifier depuis le contrôleur
    },
    data() {
      return {
        form: {
    ...
    methods: {
      async submitForm() {
        ...
        try {
          await this.$inertia.put(`/tasks/${this.task.id}`, this.form); // Envoie les données au backend
          this.$inertia.visit(route('tasks.index')); // Redirige vers la liste des tâches après mise à jour
        } catch (error) {
          if (error.response && error.response.data.errors) {
            this.errors = error.response.data.errors; // Récupère les erreurs de validation
          }
        }
      },
    },
  ...
  supprime export du code  etutilise du setup ;redirige vers la page des taches
